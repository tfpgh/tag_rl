...
from sysid.params import build_domain_and_pipeline
...
class SysIdEvaluator:
        ...
        env_config = EnvironmentConfig.from_dict(
            {
                ...
                "pipeline_randomization": {
                    "enabled": False,
                    "max_action_delay_substeps": 8,
                    "max_observation_delay_substeps": 8,
                },
    ...
    def _initial_data(
        self, initial_pose: jax.Array, initial_velocity: jax.Array
    ) -> mjx.Data:
        ...
        return self.env.forward(
            self.env.mjx_model,
            self.env._template_mjx_data.replace(qpos=qpos, qvel=qvel),
        )
    ...
    def _simulate_window(self, latent: jax.Array, window: WindowArrays) -> ScoreTerms:
        ...
        initial_data = self._initial_data(window.initial_pose, window.initial_velocity)
        ...
        def substep(
            ...
            root_qpos = data.qpos[self.qpos_slices.chaser_root]
            root_qvel = data.qvel[self.dof_slices.chaser_root]
            pose = jnp.array(
                [
                    root_qpos[0],
                    root_qpos[1],
                    jnp.arctan2(
                        ...
                        * (root_qpos[3] * root_qpos[6] + root_qpos[4] * root_qpos[5]),
                        1.0
                        - 2.0
                        * (root_qpos[5] * root_qpos[5] + root_qpos[6] * root_qpos[6]),
                    ),
                    root_qvel[0],
                    root_qvel[1],
                    root_qvel[5],
        ...
        sampled_pose = pose_history[sample_indices]
        ...
        endpoint_pos = pos_err[last_index]
        endpoint_yaw = yaw_err[last_index]
        endpoint_term = _pseudo_huber(endpoint_pos / POS_SCALE) + _pseudo_huber(
            endpoint_yaw / YAW_SCALE
        )
    ...
    def build_summary_evaluator(self, windows: WindowArrays):
        return jax.jit(jax.vmap(self._simulate_window, in_axes=(None, 0)))
    ...
    def evaluate_summary(
        ...
        total = jax.device_get(per_window_terms.total)
        position = jax.device_get(per_window_terms.position)
        yaw = jax.device_get(per_window_terms.yaw)
        linear_velocity = jax.device_get(per_window_terms.linear_velocity)
        angular_velocity = jax.device_get(per_window_terms.angular_velocity)
        endpoint = jax.device_get(per_window_terms.endpoint)
